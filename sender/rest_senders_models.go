	...
	TypeKodo: {
		{
			KeyName:       KeyKodoZone,
			ChooseOnly:    true,
			ChooseOptions: []interface{}{"华东", "华北", "华南", "北美", "东南亚"},
			Default:       "华东",
			DefaultNoUse:  false,
			Description:   "kodo存储区域(kodo_zone)",
		},
		{
			KeyName:      KeyKodoBucketName,
			ChooseOnly:   false,
			Default:      "",
			Required:     true,
			Placeholder:  "my_bucket_name",
			DefaultNoUse: true,
			Description:  "云存储存储空间(kodo_bucket_name)",
		},
		{
			KeyName:       KeyKodoFilePrefix,
			ChooseOnly:    false,
			Default:       "logkitauto/date=$(year)-$(mon)-$(day)/hour=$(hour)/min=$(min)/$(sec)",
			DefaultNoUse:  false,
			Description:   "云存储文件前缀(kodo_prefix)",
		},
		{
			KeyName:      KeyKodoAK,
			ChooseOnly:   false,
		...
		},
		{
			...
			Default:      "10",
			DefaultNoUse: false,
			Description:  "同步meta到磁盘(ft_sync_every))",
			CheckRegex:   "\\d+",
			Advance:      true,
		},
		{
			KeyName:      KeyFileFormatType,
			ChooseOnly:   true,
			Default:      "json",
			ChooseOptions: []interface{}{"json", "text", "csv"},
			DefaultNoUse: false,
			Description:  "文件保存格式(file_format_type)",
			Advance:      true,
		},
		{
			KeyName:      KeyFileFormatDelim,
			ChooseOnly:   false,
			Default:      ",",
			DefaultNoUse: false,
			Description:  "保存文件字段分割方式(file_format_delimit)",
			Advance:      true,
		},
		{
			KeyName:      KeyFileRotateSize,
			ChooseOnly:   false,
			Default:      "100",
			DefaultNoUse: false,
			Description:  "文件分割大小(file_rotate_interval)",
			CheckRegex:   "\\d+",
			Advance:      true,
			ToolTip:	  `文件达到该size后可以进行发送，单位为MB`,
		},
		{
			KeyName:      KeyFileRotateIntervalSyncSecond,
			ChooseOnly:   false,
			Default:      "600",
			DefaultNoUse: false,
			Description:  "文件分割间隔(file_kodo_rotate_interval)",
			CheckRegex:   "\\d+",
			Advance:      true,
			ToolTip:	  `loglit 会间隔 file_interval_sync_second 时间，无论收集的数据多少，都会将已收集的数据发送到kodo`,
		},
	},
}
